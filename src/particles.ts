...
export class Particles {
    ...
    spawnParticles(x: number, y: number, vec: { x: number, y: number, t: number } = null, bounce = false) {
        // if (!vec) {
        ...
        const col = this.colors[this.cidx]
        ++this.cidx
        if (this.cidx == this.colors.length) this.cidx = 0
    ...
    burst() {
        console.log('burst')
        for (let i = 0; i < 50; ++i) {
            this.spawnParticles(this.lastSpawn.x, this.lastSpawn.y, null, true)
        }
    }
